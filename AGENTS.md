...
filters) — this is deliberate: a required GitHub status check that's path-filtered can end up
permanently "pending" on a PR that never touches that path, which would block merging forever.
Running all three every time is the trade for making them safe to require.
