...
```
[docker-compose.dev.yaml](./docker-compose.dev.yaml) extends the base config with the `dev` build target, which compiles the React UI from source and installs Python deps via uv.
...
| `dev` | Local dev - builds from source using uv + Node/Yarn, includes git build info |
...
The [`docker.yaml`](../.github/workflows/docker.yaml) workflow publishes images to GHCR. It runs independently of `create-app.yaml` (the PyPI/pyinstaller release pipeline) since it builds straight from source and doesn't need to wait for a PyPI publish:
...
> [!NOTE]
> The legacy [3ll3d00d/ezbeq-docker](https://github.com/3ll3d00d/ezbeq-docker) repo and its `ghcr.io/3ll3d00d/ezbeq-docker` image keep publishing as before (still triggered from `create-app.yaml` on every PyPI release) - it isn't affected by this. `ghcr.io/3ll3d00d/ezbeq` (this folder) is the newer, source-built, multi-arch alternative; the two are independent and both stay maintained for now.
