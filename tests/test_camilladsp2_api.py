...
def test_mute_master(single_camilladsp2_client, single_camilladsp2_app, mute_op):
    config: CamillaDspSpyConfig = single_camilladsp2_app.config['APP_CONFIG']
    assert isinstance(config, CamillaDspSpyConfig)
    ensure_inited(config)

    call = single_camilladsp2_client.put if mute_op is True else single_camilladsp2_client.delete
...
def test_set_volume(single_camilladsp2_client, single_camilladsp2_app, volume):
    config: CamillaDspSpyConfig = single_camilladsp2_app.config['APP_CONFIG']
    assert isinstance(config, CamillaDspSpyConfig)
    ensure_inited(config)

    payload = {'gain': volume}
    r = single_camilladsp2_client.put(f"/api/1/devices/master/gain", data=json.dumps(payload), content_type='application/json')
    assert r
    assert r.status_code == 200
...
def test_load_known_entry_and_then_clear(single_camilladsp2_client, single_camilladsp2_app):
    config: CamillaDspSpyConfig = single_camilladsp2_app.config['APP_CONFIG']
    assert isinstance(config, CamillaDspSpyConfig)
    ensure_inited(config)

    r = single_camilladsp2_client.put(f"/api/1/devices/master/filter/CamillaDSP", data=json.dumps({'entryId': '123456_0'}),
    ...
    r = single_camilladsp2_client.delete(f"/api/1/devices/master/filter/CamillaDSP")
    assert r.status_code == 200
...
def test_load_known_entry_with_gain_and_then_clear(single_camilladsp2_client, single_camilladsp2_app):
    config: CamillaDspSpyConfig = single_camilladsp2_app.config['APP_CONFIG']
    assert isinstance(config, CamillaDspSpyConfig)
    ensure_inited(config)

    payload = {"slots": [{"id": "CamillaDSP", "gains": [{"id": "1", "value": -1.5}], "mutes": [], "entry": "123456_0"}]}
    r = single_camilladsp2_client.patch(f"/api/3/devices/master", data=json.dumps(payload), content_type='application/json')
    assert r.status_code == 200
...
def test_load_known_entry_then_load_gain_and_then_clear(single_camilladsp2_client, single_camilladsp2_app):
    config: CamillaDspSpyConfig = single_camilladsp2_app.config['APP_CONFIG']
    assert isinstance(config, CamillaDspSpyConfig)
    ensure_inited(config)
...
def test_input_gain_and_mute(single_camilladsp2_client, single_camilladsp2_app):
    config: CamillaDspSpyConfig = single_camilladsp2_app.config['APP_CONFIG']
    assert isinstance(config, CamillaDspSpyConfig)
    ensure_inited(config)
...
def test_input_gain_and_mute_on_unsupported_channel(single_camilladsp2_client, single_camilladsp2_app):
    config: CamillaDspSpyConfig = single_camilladsp2_app.config['APP_CONFIG']
    assert isinstance(config, CamillaDspSpyConfig)
    ensure_inited(config)
...
def test_load_known_entry_to_multiple_channels_with_gain_and_then_clear(multi_camilladsp2_client, multi_camilladsp2_app):
    config: CamillaDspSpyConfig = multi_camilladsp2_app.config['APP_CONFIG']
    assert isinstance(config, CamillaDspSpyConfig)
    ensure_inited(config)
    ...
    r = multi_camilladsp2_client.patch(f"/api/3/devices/master", data=json.dumps(payload), content_type='application/json')
    assert r.status_code == 200
    ...
    r = multi_camilladsp2_client.delete(f"/api/1/devices/master/filter/CamillaDSP")
    assert r.status_code == 200
