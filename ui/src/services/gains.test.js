...
describe('applyGainChange', () => {
    ...
    it('sets an input channel gain by id, leaving others untouched', () => {
        ...
        expect(result.gains).toEqual([{id: '1', value: 0}, {id: '2', value: '3.5'}]);
    });

    it('sets an input channel mute by id, leaving others untouched', () => {
        const result = applyGainChange(sampleGains(), '1', 'mute', true);
        expect(result.mutes).toEqual([{id: '1', value: true}, {id: '2', value: false}]);
    });
...
describe('mergeGains', () => {
    const gain = (masterMv, channelValue) => ({
        master_mv: masterMv, master_mute: false,
    ...
    it('recognises a committed edit as clean once the device catches up, despite the string/number mismatch', () => {
        const local = {...gain(-10, 0), gains: [{id: '1', value: '-5'}]}; // committed, stored as string
        const prevDevice = {...gain(-10, 0), gains: [{id: '1', value: -5}]}; // device now reports -5 (number)
    ...
    it('includes a channel newly reported by the device', () => {
        const local = gain(-10, 0);
        const prevDevice = gain(-10, 0);
        const newDevice = {...gain(-10, 0), gains: [{id: '1', value: 0}, {id: '2', value: 1}]};

        const merged = mergeGains(local, prevDevice, newDevice);

        expect(merged.gains).toEqual([{id: '1', value: 0}, {id: '2', value: 1}]);
    });
});

// lodash's debounce reads Date.now() internally to compute its remaining wait, which spins
// forever against vitest's fake timers (they don't advance Date in lockstep). Real timers
// with a short wait past the 200ms debounce window are used for the commitGain tests instead.
const waitPastDebounce = () => new Promise(resolve => setTimeout(resolve, 260));

describe('useGainSync', () => {
    beforeEach(() => {
        ezbeq.patchSingle.mockClear();
    });

    const device = (channelGain, masterVolume = -20) => ({
        name: 'd1',
        masterVolume,
        mute: false,
        slots: [{
    ...
    it('adopts device state wholesale on mount, then merges on same device/slot updates', () => {
        const {result, rerender} = renderHook(
            ({d, slot}) => useGainSync(d, slot, vi.fn()),
            {initialProps: {d: device(0), slot: '1'}}
        );
        expect(result.current.currentGains.gains[0].value).toBe(0);

        rerender({d: device(-3.5), slot: '1'});

        expect(result.current.currentGains.gains[0].value).toBe(-3.5);
    });

    it('does not clobber an in-flight local edit with a stale device update', () => {
        const {result, rerender} = renderHook(
            ({d, slot}) => useGainSync(d, slot, vi.fn()),
            {initialProps: {d: device(0), slot: '1'}}
        );

        act(() => result.current.updateGain('1', 'mv', '-5'));
        expect(result.current.currentGains.gains[0].value).toBe('-5');

        rerender({d: device(0), slot: '1'});

        expect(result.current.currentGains.gains[0].value).toBe('-5');
    });

    it('resets wholesale (not merges) when the selected slot changes', () => {
        const d = device(0);
        d.slots.push({id: '2', gains: [{id: '1', value: 9}], mutes: [{id: '1', value: false}], outputGains: [], outputMutes: []});
        const {result, rerender} = renderHook(
            ({slot}) => useGainSync(d, slot, vi.fn()),
            {initialProps: {slot: '1'}}
        );
        act(() => result.current.updateGain('1', 'mv', '-5')); // dirty, uncommitted edit on slot 1
    ...
    it('debounces commitGain and PATCHes only the latest value once', async () => {
        ...
        // "new" device on every render it itself triggers, looping forever. See useGainSync's
        // sync effect for why identity matters here.
        const d = device(0);
        const {result} = renderHook(() => useGainSync(d, '1', vi.fn()));
        ...
        expect(ezbeq.patchSingle).not.toHaveBeenCalled();

        await waitPastDebounce();

        expect(ezbeq.patchSingle).toHaveBeenCalledTimes(1);
        expect(ezbeq.patchSingle).toHaveBeenCalledWith('d1', '1', 'mv', '-3', '1');
    });
    ...
    it('debounces independently per field', async () => {
        ...
        expect(ezbeq.patchSingle).toHaveBeenCalledTimes(2);
        expect(ezbeq.patchSingle).toHaveBeenCalledWith('d1', '1', 'mv', '-1', '1');
        expect(ezbeq.patchSingle).toHaveBeenCalledWith('d1', 'master', 'mv', '-30', '1');
    });
    ...
    it('reports patch failures via onError', async () => {
        ...
        ezbeq.patchSingle.mockImplementationOnce(() => Promise.reject(new Error('boom')));
        const d = device(0);
        const onError = vi.fn();
        const {result} = renderHook(() => useGainSync(d, '1', onError));

        act(() => result.current.commitGain('1', 'mv', '-1'));
    ...
    it('does nothing when no device is selected', async () => {
        const {result} = renderHook(() => useGainSync(null, null, vi.fn()));

        act(() => result.current.commitGain('master', 'mv', '-5'));
