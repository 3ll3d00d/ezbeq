...
This is the iOS/iPadOS equivalent of the [Android release APK](mobile.md#android) — a way to get a
real, installed (not Expo-Go-hosted) build of the app onto your own iPhone/iPad. Apple doesn't
offer a way to install unsigned code on a real device, so every path below still uses your own
(free) Apple ID to sign the build — there's no way around that — but none of them require
enrolling in the paid Apple Developer Program ($99/year).

The trade-off for not paying: Apple caps a free-tier ("Personal Team") signature at **7 days**,
after which the app refuses to launch until it's re-signed. Every path below documents how to redo
that in under a minute once the one-time setup is done.
...
Every tag push builds the iOS app on a GitHub-hosted macOS runner (the `buildmobileios` job in
[`create-app.yaml`](https://github.com/3ll3d00d/ezbeq/blob/main/.github/workflows/create-app.yaml),
the iOS sibling of the Android `buildmobileandroid` job) and attaches two files to that tag's
[GitHub Release](https://github.com/3ll3d00d/ezbeq/releases):
...
for a real installed build (no home-screen-independent icon, needs Metro reachable each time). See
[`mobile/README.md`](https://github.com/3ll3d00d/ezbeq/blob/main/mobile/README.md#running-locally)
in the repository for the full local-dev setup. In short:
...
- **From the CI artifact** (no local build toolchain needed): download
  ...
  to push it over.
- **Building locally** (no CI artifact needed):
  ```bash
  cd mobile
...
- Push notifications: this app only ever uses *local* notifications (see [Backend
  compatibility](mobile.md#backend-compatibility)), so this doesn't affect it — but if that ever
