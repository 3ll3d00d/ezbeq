...
describe('createPatchPayload', () => {
  ...
  it('attaches the catalogue entry to the slot when an entryId is given', () => {
    const payload: any = api.createPatchPayload('1', { gains: [{ id: '1', value: 0 }] }, 'abc123');
  ...
  it('omits the slots key entirely when no slot id is given, even with an entry to load', () => {
    const payload = api.createPatchPayload(null, { master_mv: -10 }, 'abc123');
  ...
  it('omits the slots key when slotId is undefined', () => {
    const payload = api.createPatchPayload(undefined, { gains: [{ id: '1', value: 0 }] });
    expect(payload).not.toHaveProperty('slots');
  });
});
...
describe('network calls', () => {
  ...
  it('sendFilter without gains PUTs the entry id to the slot', async () => {
    ...
    await api.sendFilter('d1', 'entry-1', '1');

    expect(global.fetch).toHaveBeenCalledWith(
      `${BASE_URL}/api/1/devices/d1/filter/1`,
      expect.objectContaining({ method: 'PUT', body: JSON.stringify({ entryId: 'entry-1' }) })
    );
  });
  ...
  it('sendFilter with gains PATCHes the targeted device endpoint with the built payload', async () => {
    ...
    await api.sendFilter('d1', 'entry-1', '1', { master_mv: -10 });
  ...
  it('doGet throws with the endpoint name on a non-ok response', async () => {
    ...
    await expect(api.doGet('authors')).rejects.toThrow('EzbeqApi.getauthors failed, HTTP status 404');
  });
  ...
  it('surfaces the server message for a 5xx PATCH failure', async () => {
    ...
    await expect(api.patchSingle('d1', 'master', 'mv', '-5', '1')).rejects.toThrow(/device offline/);
  });
  ...
  it('raises a generic error for a 4xx PATCH failure without a message body', async () => {
    ...
    await expect(api.patchSingle('d1', 'master', 'mv', '-5', '1')).rejects.toThrow(/invalid request \(400\)/);
  });

  it('targets a composite device name exactly like any other device - device is just an opaque URL segment', async () => {
    (global.fetch as jest.Mock).mockResolvedValue({ ok: true, json: () => Promise.resolve({ name: 'bass_array' }) });

    await api.sendFilter('bass_array', 'entry-1', '1', { master_mv: -10 });

    expect(global.fetch).toHaveBeenCalledWith(
      `${BASE_URL}/api/3/devices/bass_array`,
      expect.objectContaining({
        method: 'PATCH',
        body: JSON.stringify({ masterVolume: -10, slots: [{ id: '1', entry: 'entry-1' }] }),
      })
  ...
  it('aborts the request once the timeout elapses', async () => {
    ...
    const pending = api.doGet('authors', 1).catch((e) => e);
